async function getMostRecentData() {
    try {
        const response = await fetch('/Scripts/temp2.json');
        
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        
        const temp2_data = await response.json();
        const recent2 = temp2_data.readings[temp2_data.readings.length - 1];
        
        console.log('=== MOST RECENT TEMPERATURE READINGS ===');
        console.log(`Sensor 2: ${recent2.temperature}°C at ${recent2.timestamp}`);
        console.log('========================================');
        
        return recent2.temperature;
